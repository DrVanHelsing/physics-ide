...
/* ── Physics constants & defaults ─────────────────────────────────────────
   ...
   every slider left untouched, or after Reset, the canvas behaves
   byte-for-byte like it did before this file had sliders at all. */
export const PX_PER_METER = 60;
export const DEFAULT_GRAVITY = 9.8; // m/s^2 — Earth's, and the slider's centre of gravity (pun intended)
...
/* Per-ball size variety, layered on top of the slider's base radius so the
   canvas keeps its varied-not-uniform look at every setting — the old
   makeBall's "+ Math.random() * 7" tail, now applied at draw/step time
   (not baked into the ball at creation) so the size slider can resize
   balls already on screen, not just ones dropped after the drag. */
...
/* No mass slider. In vacuum free-fall, acceleration is mass-independent
   ...
   this engine does not model (a control that lies differently). Gravity,
   size and bounciness are the three levers that are actually wired to real
   physics in this file, so those are the three that ship. */

/* One frame's worth of motion, used for the single click-to-drop render
   while paused (RM), and for redrawing immediately after a slider/Reset
   change while paused so the change is visible without waiting for a
   click. */
const STEP_DT = 1 / 60;
...
/** A ball's drawn/collision radius: the size slider's base plus this ball's
 *  own fixed variety offset, clamped so a dragged-to-minimum slider can
 *  never zero out or invert a ball. Exported so the physics test can check
 *  contact height the same way the renderer does. */
 ...
 * (RM) canvas redraw at a new radius/gravity/restitution setting without
 * advancing the simulation a single tick.
 *
 * Pure and side-effect-free: same inputs, same output, every time — which
 * is what makes it unit-testable without a canvas or a rAF loop.
 */
 ...
 *
 * v3: a quiet control cluster (bottom-right, low-opacity until
 * hover/focus-within) adds three real .range sliders — gravity, ball size,
 * bounciness — plus a Reset. Unlike the click-to-drop gesture, these ARE
 * keyboard-operable real form controls, each with a visible label and an
 * aria-valuetext carrying units; see the "no mass slider" comment above for
...
export default function GravityPlayground() {
  ...
  useEffect(() => {
    ...
    const render = (dt) => {
      ...
      for (let i = 0; i < balls.length; i++) {
        const b = dt > 0 ? stepBall(balls[i], dt, currentSettings, bounds, mouse) : balls[i];
        balls[i] = b;
  ...
  /* Every slider funnels through here: update the ref the render loop
     actually reads (so a running rAF loop picks the new value up on its
     very next frame, no effect re-subscription needed), mirror it into
     state so the controlled inputs and their readouts re-render, and — the
     RM/paused case — redraw once immediately so a change is visible
     without waiting for a click to single-step a frame. */
    ...
    <>
      ...
      <div className="welcome-hero__controls" role="group" aria-label="Adjust the canvas physics">
        <div className="welcome-hero__control">
          ...
          <span className="welcome-hero__control-value">{settings.gravity.toFixed(1)} m/s²</span>
          <input
            id="hero-gravity"
            className="range"
            type="range"
            ...
            onChange={(e) => updateSetting("gravity", Number(e.target.value))}
            aria-valuetext={`${settings.gravity.toFixed(1)} metres per second squared`}
          />
        </div>
        <div className="welcome-hero__control">
          <label htmlFor="hero-radius">Ball size</label>
          <span className="welcome-hero__control-value">{settings.radius.toFixed(0)} px</span>
          <input
            id="hero-radius"
            className="range"
            type="range"
            ...
            aria-valuetext={`${settings.radius.toFixed(0)} pixel radius`}
          />
        </div>
        <div className="welcome-hero__control">
          <label htmlFor="hero-restitution">Bounciness</label>
          <span className="welcome-hero__control-value">{settings.restitution.toFixed(2)}</span>
          <input
            id="hero-restitution"
            className="range"
            type="range"
            min={RESTITUTION_RANGE[0]}
            max={RESTITUTION_RANGE[1]}
            step="0.01"
            value={settings.restitution}
            onChange={(e) => updateSetting("restitution", Number(e.target.value))}
            aria-valuetext={`${settings.restitution.toFixed(2)} coefficient of restitution`}
          />
        </div>
