...
- [x] *(2a03d22, Task 2 — wiring; verified df244c2, Task 6; the production VALUE `TRUST_PROXY=true` is runbook step 10)* Configure Fastify `trustProxy` for the load balancer and verify X-Forwarded-For spoofing is closed; without it every user shares one rate-limit bucket.
...
- [x] *(the `secure` conditional has long been in auth.ts; setting the VALUE is runbook step 10)* `NODE_ENV=production` so the session cookie carries `Secure`.
...
- [ ] *(runbook step 7 — `--max-instances=1` is in the deploy command; a session step, not code)* @fastify/rate-limit's store is in-memory per instance — pin the service to max 1 instance or accept multiplied limits.
- [x] *(66e5999, Task 4)* Before real email: per-email-address throttle on `/api/auth/forgot` (mail-bomb shape) and wrap the mailer send so a mail failure cannot become a user-existence oracle.
