...
|---|---|---|---|
| F1 | All portal screens (class tabs, profile, admin) | rough | Content pinned upper-left at full-screen: `.page-body` max-width 1000px with no centering (platform.css:169-173) — full-screen is a requirement; wants centered shell + wider cap for table screens. (User, 2026-08-28) — FIXED 28a8c16 (1200px centered, 1600px wide screens, in-browser measured) |
| F2 | Portal navigation | rough | No back affordance on drill-down pages; the logo goes home, not back. Browser back works but an in-header back link is wanted. (User, 2026-08-28) — FIXED 28a8c16+695604f (shared BackLink on every drill-down + /profile; redundant defaults stand down) |
| F3 | IDE — block editor | wrong | Windows scroll gestures ZOOM the canvas instead of scrolling (`zoom.wheel: true`, no `move.wheel` — BlocklyWorkspace.js:402). Wanted: wheel scrolls, Ctrl+wheel/pinch or the buttons zoom. (User, 2026-08-28) — FIXED 90be9c6, wheel-scroll verified in-browser |
