...
|---|---|---|
| Hosting | Static SPA on Vercel or Cloudflare Pages; *amended 1 Sep 2026 (Plan 9 Task 10a, change protocol):* **Firebase Hosting fronting Cloud Run is also permitted** — the app is same-origin by construction (every call a relative `/api/...` path, `credentials: "same-origin"`, `SameSite=Lax`, no CORS plugin), so the static host must be able to front the API. *Amended 2 Sep 2026 (Plan 9 Stage E, decided by the user at the provisioning session):* **the Cloud Run container itself serves the built SPA same-origin (`STATIC_DIR`)** — Firebase Hosting rewrites do not support `africa-south1`, and the container path keeps single-cloud, in-country serving at zero standing cost. `firebase.json` and `vercel.json` remain in-tree as valid alternates. | Free tier, no cold starts, fits the "all free services" constraint. |
