...
**What this amendment does not lift.** Design §9's exclusions stand: the notification bell and per-user notification preferences, peer sharing and the §8.3 attribution ledger, admin data requests, rubric marking, **real email delivery** (mail goes to the pretend inbox — the admin console's Emails tab), the GCP port, BlobStore, and websockets / live co-editing. The exclusion-list bullets above that were lifted on 18 August 2026 stay lifted; every other v1 exclusion stands unchanged.
