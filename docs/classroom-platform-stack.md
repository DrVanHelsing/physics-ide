...
- **React 18 stays.** The IDE core — Blockly workspace, Monaco editor, GlowScript iframe runner, Arquero + Observable Plot data panel, the debug tools — moves into `frontend/` essentially untouched. Nothing about the simulation experience is rebuilt.
- ⚖️ **Build tool: migrate from Create React App to Vite.** CRA (what the project uses today) is unmaintained and was already flagged in the product contract as a future migration. The restructure is the natural moment: Vite is the current standard, the migration for this app is mechanical, and dev startup drops from ~20s to ~1s. The existing Jest tests move to Vitest (a near-drop-in). *Alternative: keep CRA and defer — carries a dead build tool into a new codebase. I recommend migrating now.*
- **React Router** for the ~24 portal screens (the app today is single-screen and has no router).
